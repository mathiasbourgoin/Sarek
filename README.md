...
- **Machine-checked formal models** — **94 theorems are checked to agree with the code that actually ships**, not merely with the Rocq model: 50 (the PTX layout theory) via its own extraction run directly against production, exhaustively over every record/variant shape up to a bound plus randomised beyond it; 44 (two type-safety modules) via QCheck differential testing against the real typer/unifier, sampled over 2000/1000 random cases — sampling, not exhaustive, so treat it as strong agreement evidence rather than a transported proof. `scripts/check-production-link.py` derives these numbers by mechanically verifying each production-link claim against its named test file's real content (a claim whose test regresses fails the gate) and reading theorem counts from the committed, drift-checked `formal/*/proof-ledger.json` files. Run `scripts/check-production-link.py` directly (no rebuild, no container, seconds not minutes) to reproduce them yourself; `scripts/check-formal-proofs.sh` runs the same check after a from-scratch `coqchk`-verified rebuild, which is the stronger guarantee but also destroys uncommitted work under `formal/` on its way out, so prefer the standalone script unless you specifically need the rebuild. No gate compares this sentence's digits against either script's output, so treat the numbers above as a snapshot, not a live figure. The three Rocq projects under `formal/` (`convergence-safety`, `type-safety`, `codegen-ptx`) additionally carry model-only proofs — machine-checked against the Rocq model itself, with no mechanical link to shipped code — listed separately in `docs/formal/rocq-value-ledger.md`, which explains what those do and do not cover
...
- **Proof-to-production link is uneven.** The *aggregate byte layout* half is
  ...
  files `Extract.v` produces are regenerated by every proof run, are not tracked
  (backlog-188), and are linked by nothing. On that half the *production*
  emitter is not compared to the model at all — the mirror is compared to the
  mirror's own spec. The only cases in that suite that invoke
