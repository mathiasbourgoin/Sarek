...
# a git command. See the header comment: this is not defensive habit, it is
# how this project's git-dir-clobber incident happened.
require_nonempty() {
  if [ -z "$2" ]; then
...
# into one project's subshell, where it appends a drift line to a tracked
# file (standing in for extraction rewriting real .ml/.mli files) and then
# fails -- which is exactly where the trap under test fires, without
# compiling anything.
...
case "$1" in
  --version)
    ...
    ;;
  makefile)
    # Stand-in for the real script's documented drift: a rebuild rewrites
    # tracked files. Then fail, before anything is actually compiled -- all
    # this test needs is for the subshell's EXIT trap to fire.
    echo "stub-build-drift" >> driftfile.txt
    exit 1
    ;;
  *)
    exit 1
...
make_fixture() {
  ...
  mkdir -p "$dir/scripts" "$dir/formal/testproj/theories"
...
# The exact regression: a developer's own uncommitted work must survive.
...
if printf '%s' "$out" | grep -qF 'skipping the tree restore'; then
  echo "  PASS: case-a: gate names why it is not restoring (flag observed, not accidental)"
  pass=$((pass + 1))
else
  echo "  FAIL: case-a: edit survived but the gate never said it skipped the restore --" \
       "cannot tell a real fix from the trap simply never running"
  fail=$((fail + 1))
fi
...
before="$(cat "$d/formal/testproj/driftfile.txt")"
out="$(run_gate "$d")"
after="$(cat "$d/formal/testproj/driftfile.txt")"
