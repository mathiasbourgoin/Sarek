...
for f in sources:
    try:
        ...
        continue
    # Join a comment line broken mid-token: a trailing "-" followed by the next
    # line's comment prefix is one path, not two. Line numbers are reported
    # from the ORIGINAL text, so a finding still points at a real line.
    text = re.sub(r"-\n\s*\*?\s*", "-", raw)
