...
test_negative:
	...
	@out=$$(mktemp); pat='needs a type annotation'; dune build --profile=negative sarek/tests/negative/neg_test_module_const_no_type.cma > "$$out" 2>&1; if grep -qF "$$pat" "$$out"; then echo "  PASS: unannotated module constant rejected"; else echo "  FAIL: expected [$$pat]; got:"; cat "$$out"; rm -f "$$out"; exit 1; fi; rm -f "$$out"
	@echo "Testing dropped kernel-module item rejected (backlog-192)..."
	@out=$$(mktemp); pat='exception declaration is not supported in a kernel module'; dune build --profile=negative sarek/tests/negative/neg_test_module_item_dropped.cma > "$$out" 2>&1; if grep -qF "$$pat" "$$out"; then echo "  PASS: dropped kernel-module item rejected"; else echo "  FAIL: expected [$$pat]; got:"; cat "$$out"; rm -f "$$out"; exit 1; fi; rm -f "$$out"
	@echo "Testing non-structure payload module rejected (backlog-192)..."
	@out=$$(mktemp); pat='only a literal structure is supported here'; dune build --profile=negative sarek/tests/negative/neg_test_module_not_structure.cma > "$$out" 2>&1; if grep -qF "$$pat" "$$out"; then echo "  PASS: non-structure payload module rejected"; else echo "  FAIL: expected [$$pat]; got:"; cat "$$out"; rm -f "$$out"; exit 1; fi; rm -f "$$out"
	...
	@out=$$(mktemp); pat='a type alias'; dune build --profile=negative sarek/tests/negative/neg_test_payload_type_alias.cma > "$$out" 2>&1; if grep -qF "$$pat" "$$out"; then echo "  PASS: payload type alias rejected"; else echo "  FAIL: expected [$$pat]; got:"; cat "$$out"; rm -f "$$out"; exit 1; fi; rm -f "$$out"
	@echo "Testing GADT constructor rejected (backlog-192)..."
	@out=$$(mktemp); pat='GADT-style constructor declaration'; dune build --profile=negative sarek/tests/negative/neg_test_payload_gadt.cma > "$$out" 2>&1; if grep -qF "$$pat" "$$out"; then echo "  PASS: GADT constructor rejected"; else echo "  FAIL: expected [$$pat]; got:"; cat "$$out"; rm -f "$$out"; exit 1; fi; rm -f "$$out"
	...
	@out=$$(mktemp); pat='or-pattern'; dune build --profile=negative sarek/tests/negative/neg_test_pattern_table_or.cma > "$$out" 2>&1; if grep -qF "$$pat" "$$out"; then echo "  PASS: pattern table names the construct"; else echo "  FAIL: expected [$$pat]; got:"; cat "$$out"; rm -f "$$out"; exit 1; fi; rm -f "$$out"
	...
	@out=$$(mktemp); pat='`type nonrec` is not supported'; dune build --profile=negative sarek/tests/negative/neg_test_payload_type_nonrec.cma > "$$out" 2>&1; if grep -qF "$$pat" "$$out"; then echo "  PASS: type nonrec rejected"; else echo "  FAIL: expected [$$pat]; got:"; cat "$$out"; rm -f "$$out"; exit 1; fi; rm -f "$$out"
