...
test_negative:
	...
	@echo "Testing annotation arity rejected (backlog-192)..."
	@out=$$(mktemp); pat='fewer arrows than the function has parameters'; dune build --profile=negative sarek/tests/negative/neg_test_annotation_arity.cma > "$$out" 2>&1; if grep -qF "$$pat" "$$out"; then echo "  PASS: annotation arity rejected"; else echo "  FAIL: expected [$$pat]; got:"; cat "$$out"; rm -f "$$out"; exit 1; fi; rm -f "$$out"
