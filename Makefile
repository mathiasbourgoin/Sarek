...
test_negative:
	...
	@out=$$(mktemp); pat='Cannot unify types'; dune build --profile=negative sarek/tests/negative/neg_test_local_annotation_read.cma > "$$out" 2>&1; if grep -qF "$$pat" "$$out"; then echo "  PASS: local let annotation is read"; else echo "  FAIL: expected [$$pat]; got:"; cat "$$out"; rm -f "$$out"; exit 1; fi; rm -f "$$out"
	@echo "Testing helper return annotation is read (backlog-192)..."
	@out=$$(mktemp); pat='Cannot unify types'; dune build --profile=negative sarek/tests/negative/neg_test_helper_return_read.cma > "$$out" 2>&1; if grep -qF "$$pat" "$$out"; then echo "  PASS: helper return annotation is read"; else echo "  FAIL: expected [$$pat]; got:"; cat "$$out"; rm -f "$$out"; exit 1; fi; rm -f "$$out"
	@echo "Testing payload helper return annotation is read (backlog-192)..."
	@out=$$(mktemp); pat='Cannot unify types'; dune build --profile=negative sarek/tests/negative/neg_test_module_helper_return_read.cma > "$$out" 2>&1; if grep -qF "$$pat" "$$out"; then echo "  PASS: payload helper return annotation is read"; else echo "  FAIL: expected [$$pat]; got:"; cat "$$out"; rm -f "$$out"; exit 1; fi; rm -f "$$out"
