...
(** Software f64 transcendentals for the PTX backend.
    ...
    no instruction for: sin, cos, tan, asin, acos, atan, atan2, exp, expm1, log,
    log10, log1p, pow, sinh, cosh, tanh. The PTX emitter registers them on
    demand and inlines them through the existing EApp machinery. Precise tier:
    max relative error ≤ 1e-12 on the documented domains (see the .ml header for
    domains and coefficient provenance). *)
