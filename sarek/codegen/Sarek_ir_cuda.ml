...
let rec gen_expr st buf = function
  ...
  | EIntrinsic (path, name, args) ->
      Dispatch.gen_intrinsic (cuda_backend st) buf path name args
  | ECast (TFloat16, e) ->
...
let rec gen_stmt st buf indent = function
  ...
  | SAssign (lv, e) -> (
      ...
      match e with
      | ERecord (_, fields) -> gen_record_assign st buf indent lv fields
      | _ ->
  ...
  | SLet (v, EArrayCreate (elem_ty, size, mem), body) ->
      ...
      Buffer.add_string buf indent ;
      gen_array_decl st buf v elem_ty size mem ;
      gen_stmt st buf indent body
  | SLet (v, e, body) ->
      Buffer.add_string buf indent ;
      Buffer.add_string buf (cuda_type_of_elttype v.var_type) ;
...
and gen_record_assign st buf indent lv fields =
  List.iter
    (fun (fname, fexpr) ->
      Buffer.add_string buf indent ;
      gen_lvalue st buf lv ;
...
and gen_match_case st buf indent scrutinee pattern body =
  ...
  | PConstr (cname, bindings) -> (
      (* Lookup constructor types from the kernel's variants *)
...
and gen_array_decl st buf v elem_ty size mem =
