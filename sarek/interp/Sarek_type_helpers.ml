...
module type HELPERS = sig
  ...
  (** Immediate field names, in declaration order — the SAME order {!to_values}
      ...
      Generated by the PPX from the same [labels] list that generates
      [to_values] and [get_field], so the three cannot drift. Empty for a
      variant, which has no named fields. *)
