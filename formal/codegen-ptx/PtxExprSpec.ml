...
| IEBinop (op, e1, e2) ->
  if is_cmp_op op
  then PtxCmp ((ir_binop_to_ptx_cmp op), (emit_ast_expr e1),
         (emit_ast_expr e2))
  else PtxBinop ((ir_binop_to_ptx_binop op), (emit_ast_expr e1),
         (emit_ast_expr e2))
| IEArrayRead (i, base_e, idx_e) ->
  (match i with
   | MS_Global ->
     PtxGlobalRead (PtxBinop (PAdd, (emit_ast_expr base_e),
       (emit_ast_expr idx_e)))
   | MS_Shared ->
     PtxSharedRead (PtxBinop (PAdd, (emit_ast_expr base_e),
       (emit_ast_expr idx_e))))
