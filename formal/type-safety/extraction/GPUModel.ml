...
let rec infer_fun_type env = function
          ...
          | TFun (l, ret_ty) -> (
              ...
              | p_ty :: l0 -> (
                  ...
                  | [] -> (
                      ...
                      | Inl targ ->
                          if sarek_type_eq_dec targ p_ty then Inl ret_ty
  ...
  | FELetRec (fn_name, p_name, p_ty, ret_ty, body, cont) -> (
      ...
      | Inl tbody ->
          if sarek_type_eq_dec tbody ret_ty then
            infer_fun_type ((fn_name, fn_ty) :: env) cont
...
(** val infer_mut_type : type_env -> mut_env -> mut_expr -> (sarek_type,
    mut_error) sum **)
      ...
      | Inl tv -> (
          ...
          | Some tdecl ->
              if is_mutable mu name then
                if sarek_type_eq_dec tv tdecl then Inl (TPrim TUnit)
                else Inr (MEAssignMismatch (tdecl, tv))
              else Inr (MEImmutable name)
          | None -> Inr (MEUnbound name))
      | Inr err -> Inr err)

(** val lookup_constr : (string * sarek_type option) list -> string ->
    sarek_type option option **)
...
(** val branch_body_env : type_env -> sarek_type option -> string option ->
    type_env **)

let branch_body_env env payload bvar =
  match payload with
  | Some pty -> ( match bvar with Some v -> (v, pty) :: env | None -> env)
  | None -> env

(** val check_branches : (type_env -> mut_env -> pat_expr -> (sarek_type,
    pat_error) sum) -> type_env -> mut_env -> (string * sarek_type option) list
    -> sarek_type -> ((string * string option) * pat_expr) list -> (unit,
    pat_error) sum **)
          ...
          | TVariant (_, constrs) -> (
              ...
              | p :: rest -> (
                  ...
                  | Some payload -> (
                      match
                        infer_pat_type
                          (branch_body_env env payload bvar)
                          mu
                          body
                      with
                      | Inl result_ty -> (
...
type constr_expr =
  | CEPat of pat_expr
  | CERecord of
      string * (string * sarek_type) list * (string * constr_expr) list
  | CEConstr of
      string * (string * sarek_type option) list * string * constr_expr option

(** val check_fields : (type_env -> mut_env -> constr_expr -> (sarek_type,
    constr_error) sum) -> type_env -> mut_env -> (string * sarek_type) list ->
...
let rec check_fields infer_f env mu declared = function
  ...
  | p :: rest -> (
      ...
      | Some declared_ty -> (
          ...
          | Inl got_ty ->
              if sarek_type_eq_dec got_ty declared_ty then
                check_fields infer_f env mu declared rest
              else Inr (FieldTypeMismatch (fname, declared_ty, got_ty))
          | Inr err -> Inr err)
      | None -> Inr (UnknownField fname))

(** val infer_constr_type : type_env -> mut_env -> constr_expr -> (sarek_type,
    constr_error) sum **)
      ...
      | Some payload -> (
          ...
          | Some pty -> (
              ...
              | Some a -> (
                  ...
                  | Inl got ->
                      if sarek_type_eq_dec got pty then
                        Inl (TVariant (tyname, constrs))
...
(** val infer_special_type : type_env -> mut_env -> special_expr -> (sarek_type,
    special_error) sum **)
...
let rec infer_special_type env mu = function
  ...
  | SEReturn (allowed, body) ->
      if allowed then infer_special_type env mu body
      else Inr EarlyReturnNotAllowed
  | SECreateArray (size, elt, mem) -> (
      match infer_special_type env mu size with
      | Inl sz_ty ->
          if sarek_type_eq_dec sz_ty (TPrim TInt32) then Inl (TArr (elt, mem))
          else Inr (ArraySizeNotInt sz_ty)
      | Inr err -> Inr err)
  | SETyped (body, annot) -> (
      match infer_special_type env mu body with
      | Inl got ->
          if sarek_type_eq_dec got annot then Inl annot
          else Inr (TypeAnnotMismatch (annot, got))
      | Inr err -> Inr err)

type gpu_error =
  | GSpecialErr of special_error
  | SharedNotArray of sarek_type
  | SharedNotShared of sarek_type
  | SuperstepBodyNotUnit of sarek_type

type gpu_expr =
  | GESpecial of special_expr
  | GELetShared of string * sarek_type * gpu_expr
  | GESuperstep of gpu_expr

(** val infer_gpu_type : type_env -> mut_env -> gpu_expr -> (sarek_type,
    gpu_error) sum **)

let rec infer_gpu_type env mu = function
  | GESpecial se -> (
  ...
  | GELetShared (name, ty, body) -> (
      ...
      | TArr (_, m) -> (
          match m with
          | Shared -> infer_gpu_type ((name, ty) :: env) mu body
          | _ -> Inr (SharedNotShared ty))
      | _ -> Inr (SharedNotArray ty))
  | GESuperstep body -> (
      match infer_gpu_type env mu body with
      | Inl bt ->
          if sarek_type_eq_dec bt (TPrim TUnit) then Inl (TPrim TUnit)
          else Inr (SuperstepBodyNotUnit bt)
      | Inr err -> Inr err)
