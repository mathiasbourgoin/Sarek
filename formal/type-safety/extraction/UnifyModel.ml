...
let rec subst_lookup s id =
  ...
  | [] -> None
  | p :: rest ->
    let (id', t) = p in if (=) id id' then Some t else subst_lookup rest id
...
let follow fuel s t = match t with
...
(** val unify_fun :
    int -> pre_subst -> pre_type -> pre_type -> pre_subst option **)
  ...
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    ...
    | PVar id ->
      (match follow n s t2 with
       | PVar id2 ->
         if (=) id id2 then Some s else Some ((id, (PVar id2)) :: s)
       | x -> if occurs_in n s id x then None else Some ((id, x) :: s))
    | PPrim p1 ->
      (match follow n s t2 with
       | PVar id ->
         if occurs_in n s id (PPrim p1)
         then None
    ...
    | PTuple ts1 ->
      (match follow n s t2 with
       | PVar id ->
         if occurs_in n s id (PTuple ts1)
         then None
         else Some ((id, (PTuple ts1)) :: s)
       | PTuple ts2 ->
         if negb ((=) (length ts1) (length ts2))
         then None
...
let rec apply_subst fuel s t = match t with
| PVar id -> follow_pvar fuel s id
