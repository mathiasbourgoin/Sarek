...
| EIf (c, t, el) ->
  (&&) ((&&) (barrier_free c) (barrier_free t)) (barrier_free el)
...
let empty_dim_usage =
  { uses_x = false; uses_y = false; uses_z = false; uses_block_dim = false;
    uses_grid_dim = false; uses_thread_idx = false; uses_block_idx = false;
    uses_shared_mem = false }

(** val merge_dim_usage : dim_usage -> dim_usage -> dim_usage **)

let merge_dim_usage a b =
  { uses_x = ((||) a.uses_x b.uses_x); uses_y = ((||) a.uses_y b.uses_y);
    uses_z = ((||) a.uses_z b.uses_z); uses_block_dim =
    ((||) a.uses_block_dim b.uses_block_dim); uses_grid_dim =
    ((||) a.uses_grid_dim b.uses_grid_dim); uses_thread_idx =
