...
let probe_report () =
  ...
  Hashtbl.iter (fun cls count ->
    Printf.printf "  mode=%-10s var=%-5b bar=%-5b dcf=%-5b depth=%d  count=%d\n"
      (match cls.mode with Converged -> "Converged" | Diverged -> "Diverged")
      cls.varying_root cls.has_barrier cls.has_diverging cls.depth count
  ) table;
