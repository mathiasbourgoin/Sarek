...
let rec is_varying_inline = function
  | EVary -> true | ELit | EBarrier -> false
...
let rec barrier_free_inline = function
  ...
  | ELet (v, b)       -> barrier_free_inline v && barrier_free_inline b
...
let rec has_diverging_cf_inline = function
  ...
  | ELet (v, b)       -> has_diverging_cf_inline v || has_diverging_cf_inline b
...
let rec check_inline m e =
  ...
  | ELet (v, b)      -> check_inline m v @ check_inline m b
...
let rec to_extracted : expr -> M.expr = function
  ...
  | ELet (v, b)       -> ELet (to_extracted v, to_extracted b)
