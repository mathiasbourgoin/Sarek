...
let test_merge_dim_idem =
  Test.make ~name:"merge_dim_idempotent" ~count:2000 gen_dim (fun a ->
      merge_dim_usage a a = a)

(* 1d. merge_dim right identity *)
let test_merge_dim_empty_r =
  Test.make ~name:"merge_dim_empty_r" ~count:2000 gen_dim (fun a ->
      merge_dim_usage a empty_dim_usage = a)

(* 1e. merge_dim left identity *)
let test_merge_dim_empty_l =
  Test.make ~name:"merge_dim_empty_l" ~count:2000 gen_dim (fun a ->
...
let test_diverged_clean_iff_bf =
  Test.make
    ~name:"diverged_clean_iff_barrier_free"
    ~count:2000
    gen_expr
    (fun e -> check Diverged e = [] = barrier_free e)

(* 4. mode_monotone: every error in Converged is also in Diverged *)
let test_mode_monotone =
  Test.make ~name:"mode_monotone" ~count:2000 gen_expr (fun e ->
...
let test_varying_if_flags =
  Test.make
    ~name:"varying_if_flags_barriers"
    ~count:2000
    ...
    (fun (ctx, cond, then_, else_) ->
      ...
      if varyc && no_bf then check ctx (EIf (cond, then_, else_)) <> []
      else true)

(* 6. cdcf_check_agreement *)
let test_cdcf_agreement =
  Test.make ~name:"cdcf_check_agreement" ~count:2000 gen_expr (fun e ->
      if not (has_diverging_cf e) then check Converged e = [] else true)
...
let test_f02_let_alias_env =
  Test.make
    ~name:"f02_let_alias_env_catches_barrier"
    ~count:2000
...
let test_f02_env_mode_monotone =
  Test.make ~name:"f02_env_mode_monotone" ~count:2000 gen_expr (fun e ->
...
let test_warp_diverged_error =
  Test.make ~name:"warp_diverged_error" ~count:2000 gen_expr (fun e ->
      ...
      List.for_all
        (fun err -> List.mem err (check_warp Diverged e))
...
let test_superstep_outer_diverged_error =
  Test.make
    ~name:"superstep_outer_diverged_error"
    ~count:2000
...
let test_superstep_no_entry_error_converged =
  Test.make
    ~name:"superstep_no_entry_error_converged"
    ~count:2000
...
let test_superstep_body_errors_propagate =
  Test.make
    ~name:"superstep_body_errors_propagate"
    ~count:2000
