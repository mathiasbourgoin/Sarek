...
let mk_barrier () =
  mk (TEIntrinsicFun (barrier_ref, Some Sarek_core_primitives.ConvergencePoint, []))
...
let test_direct_varying_if_catches () =
  let cond =
    mk ~ty:t_bool
      (TEBinop (Gt, mk_thread_idx_x (), mk_int ()))
  in
...
let test_let_varying_no_barrier_is_clean () =
  ...
  let errors = check prog in
  if has_barrier_error errors then
    failwith "FALSE-POSITIVE: let tid=tid_x; if tid < n then () — no barrier, must be clean"
...
let test_superstep_varying_if_no_barrier_is_clean () =
  ...
  let te_tid = mk ~ty:t_int (TEVar ("tid", n)) in
  let cond = mk ~ty:t_bool (TEBinop (Lt, te_tid, mk_int ())) in
  let body = mk (TELet ("tid", n, mk_thread_idx_x (),
    mk (TEIf (cond, mk TEUnit, None)))) in
  let cont = mk TEUnit in
  let prog = mk (TESuperstep ("s", false, body, cont)) in
  let errors = check prog in
  if has_barrier_error errors then
    failwith
      "FALSE-POSITIVE: superstep { let tid=tid_x in if tid < n then () } — \
       no barrier inside, implicit barrier is safe, must be clean"
...
let test_f02_let_alias_if () =
  ...
  if not (has_barrier_error errors) then
    failwith
      "F-02 FAIL: let x = thread_idx_x in if x > 0 then barrier() — \
       checker missed diverged barrier (expected BarrierError)"
...
let test_f02_let_alias_while () =
  ...
  let prog =
    mk
      (TELet
         ("x", x_id, mk_thread_idx_x (), mk (TEWhile (cond, mk_barrier ()))))
  in
  let errors = check prog in
  if not (has_barrier_error errors) then
    failwith
      "F-02 FAIL: let x = thread_idx_x in while x > 0 — \
       checker missed diverged barrier"
...
let test_f02_double_alias () =
  ...
  let inner_let = mk (TELet ("y", y_id, mk ~ty:t_int (TEVar ("x", x_id)), inner_if)) in
...
let test_f01_superstep_in_diverged_if () =
  let cond =
    mk ~ty:t_bool (TEBinop (Gt, mk_thread_idx_x (), mk_int ()))
  in
  ...
  if not (has_barrier_error errors) then
    failwith
      "F-01 FAIL: superstep inside diverged if — \
       implicit end-of-superstep barrier not flagged"
...
let () =
  let tests = [
    ...
    ("no-fp: superstep varying if no barrier",  test_superstep_varying_if_no_barrier_is_clean);
