...
T3-SEMANTIC MILESTONE LOCKED 2026-06-15 (originally via PR #201, which targeted the stale `master` branch; ported to `main` 2026-07-01 with proof artifacts regenerated and coqchk re-run on `main`'s tree). Regenerated proof-ledger.json at lock: 78 theorems, 0 falsified, 0 open assumptions (earlier "86 ledger items" figures in the dated entries above counted the pre-restructure ledger format).
