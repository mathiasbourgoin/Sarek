...
> **None of this project's 111 theorems is linked to production (backlog-201).**
...
T3-SEMANTIC MILESTONE LOCKED 2026-06-15. Checked 2026-07-30: this parenthetical used to read "originally via PR #201, which targeted the stale `master` branch; ported to `main` 2026-07-01" — but there is no `master` or `origin/master` branch in this repository, so that is not a ref a fresh clone can resolve and it is dropped rather than repeated. What *is* reachable: `891cb2b4` ("feat(formal): add convergence-safety CMBT apparatus", 2026-06-11) scaffolded this project and `c3f2578b` ("docs(formal): T3-SEMANTIC milestone lock on main — both projects", 2026-07-02) recorded this lock on `main`; both are ancestors of `origin/main`. The port to `main` regenerated the proof artifacts and re-ran `coqchk` on `main`'s tree. Regenerated proof-ledger.json at lock: 78 theorems, 0 falsified, 0 open assumptions (earlier "86 ledger items" figures in the dated entries above counted the pre-restructure ledger format).
Non-vacuousness gate passed: 6 Example witnesses in ConvergenceSemantics.v, M1+M2 mutation tests green.
