...
- Breaking: `Sarek_type_helpers.HELPERS` gained a required `val field_names :
  ...
  therefore copy. Every in-tree implementation is generated by the
  `[@@sarek.type]` PPX from the same label list, so the correspondence holds by
  construction there; a hand-written implementation outside this repository has
  to add the list, and carries the ordering obligation itself. There is no
  non-breaking spelling — a module type cannot carry a default, and the index is
  not derivable from the rest of the signature.
