...
- **Fixed as of 2026-07-02** (was: `ThreadPool.worker_fn` could deadlock waiters on an unprotected task exception): work execution is now wrapped in `try ... with exn -> Some exn` (`sarek/sarek/Sarek_cpu_runtime_pools.ml:367-386`), the pending-worker decrement/broadcast always runs afterward (`:398`), and the first captured exception is recorded on `pool.first_error` (`:390-393`) for the caller to re-raise after drain.
- **Fixed 2026-07-03 (merged, PR #217):** `ParallelPool.worker_fn`'s exception-hang is closed — `work_fn` is wrapped, the pending-worker decrement/signal always runs, and the first error (with raw backtrace) is recorded on `pool.first_error` and re-raised by the waiter via `Printexc.raise_with_backtrace`. The same PR fixed a deeper `DomainPool` bug in this file: `create` returned `{pool with domains}`, splitting the record identity so workers mutated one `active_tasks` cell while `wait_all` read another — `wait_all` could return while the last block still executed, silently dropping its writes (reproduced at 96 domains; regression test `sarek/tests/e2e/test_domain_pool_stress.ml`). All lazy pool getters are now `Fun.protect`-guarded so a raise during pool creation cannot leave the init mutex locked. `SAREK_DOMAIN_COUNT` forces the domain/worker count for oversubscription stress-testing.
...
- **2026-07-02:** `ThreadPool` worker exception propagation fixed (try/with + `first_error`). **2026-07-03 (PR #217):** `ParallelPool` hang fixed; `DomainPool` record-aliasing work-drop fixed; backtraces preserved on re-raise; pool init exception-safe (`Fun.protect`).
...
- Worker exception propagation for `ParallelPool` (still open; `ThreadPool` is covered/fixed as of 2026-07-02 — regression test recommended to lock in the fix).
...
- Apply the same try/with + `first_error` pattern used to fix `ThreadPool` (2026-07-02) to `ParallelPool.worker_fn`'s `process_chunks`, so pending counters and condition variables are updated even when `work_fn` raises.
- `ThreadPool` already captures the first worker exception and exposes it via `pool.first_error`; extend the same mechanism to `ParallelPool` and re-raise in the caller after the pool drains.
