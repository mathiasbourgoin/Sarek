...
- `sarek/tests/codegen_golden/test_wgsl_array_equality_refusal.ml` (backlog-217): WGSL-only refusal of whole-value `=`/`<>` on a `TVec`/`TArray` operand (a kernel-vector parameter or a local array). Not closed by backlog-194/#401 — `Sarek_typer.reject_aggregate_equality` and `Sarek_types.is_uncomparable_operand_typ` deliberately exclude `TVec`/`TArray`, because `src = dst` on that pointer-shaped C-family value emits `(src == dst)`, and clang -x cl / glslangValidator (`-V`) both accept it at exit 0 (established by hand before this fix, generating this exact kernel's OpenCL/GLSL output and feeding it to each compiler; this test's `test_c_family_still_accepts` does NOT invoke a compiler — it only pins that the four emitters still print the operator unchanged). WGSL does not generalise: naga 30.0.0 has no equality operator on `array<T>`/`array<T, N>` and rejects the identical shape with `Incompatible operands: Equal(Array { .. }, _)`. Reproduced through the full frontend (`Sarek_transpile.of_source WGSL "fun (a : float32 vector) (b : float32 vector) ... -> if a = b then ..."`) before this fix landed, then pinned at the IR level like the sibling `test_etuple_backend_refusal.ml`. Refused in `Sarek_ir_wgsl.ml`'s `EBinop (Eq | Ne, ..)` arm via the new `is_array_shaped_operand` predicate, which only matches a bare `EVar` of vector/array type — reading an element (`EArrayRead`) still lowers to ordinary scalar equality and is untouched (`test_wgsl_accepts_element_eq`).
