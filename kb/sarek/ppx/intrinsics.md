...
- **Reclassified 2026-07-02 (merged, PRs #211/#213) — no longer a conflict, deliberately unaffected by the `f8d436a9` float-default spec change.** `"float"` still maps to `float64` in intrinsic PPX conversion at `sarek/ppx_intrinsic/Sarek_ppx_intrinsic.ml:79-94` (verified unchanged at HEAD `618768b7`). Kernel-body bare `float` was changed to default to `float32` (human decision, see `kb/sarek/ppx/types.md`), but this intrinsic-*registration* resolver is a separate mechanism that the `Sarek_float64`/`Float64.ml` stdlib relies on to mean float64 in its own registered signatures; per the `f8d436a9` commit message, it was intentionally left untouched — changing it would corrupt the float64 math library. This is a two-mechanism split by design, not residual drift.
