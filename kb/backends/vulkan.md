...
- `SNative` is refused outright (`sarek/codegen/Sarek_ir_glsl.ml:1309`, the shared `native_block_refusal` from `spoc/ir/Sarek_ir_codegen.ml:147`), not emitted as a comment — `sarek-vulkan/Sarek_ir_glsl.ml` just re-exports that generator (`sarek-vulkan/Sarek_ir_glsl.ml:9`).
