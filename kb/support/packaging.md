...
- **fixed 2026-07-02 (merged)** — `Makefile` target `opam` previously appended `echo 'available: [ os = "linux" ]' >> sarek.opam` and the same line to a nonexistent `sarek_ppx.opam` on every run (non-idempotent, and no longer accurate now that Metal targets macOS). Commit `e567635a` ("stop opam target from corrupting sarek.opam") reduced the target to `dune build @install` (confirmed: `opam:\n\tdune build @install`, no trailing `echo`/`>>` lines) and added `scripts/check-opam-clean.sh` (60 lines) as a regression guard asserting idempotence, absence of an `available:` line, and no `sarek_ppx.opam` references. This guard is wired into the test chain (commit `2b384bad`, "wire check-opam-clean into test chain").
- **fixed 2026-07-02 (merged)** — `make check` previously depended on undefined targets (`check: all install install_sarek samples test`, referencing a nonexistent `install_sarek`/`samples`/`Samples/` tree) and always failed with "No rule to make target." Commit `5df0890b` ("repair check target and remove phantom Samples targets") narrowed it to `check: all test` (confirmed at `Makefile:379`), and deleted the `test_samples_cuda` target, which referenced the same removed `Samples/` tree and could never succeed even with a CUDA toolchain present.
- Root `dune` lists `test_verification` (`dune:9`, corrected 2026-07-02 from the prior `:8-10` citation — the file is only 11 lines: `dirs` plus one directory name per line); this directory was not present in the initial support-scope directory listing. Marked uncertain because Dune may tolerate absent dirs differently by version/config, but it is suspicious. **Missing from prior inventory:** the same `dirs` stanza also lists `formal` as its last entry (`dune:11`), which was previously undocumented in this KB.
...
- **FIXED (verified 2026-07-02):** `sarek-metal.opam`'s Dune version constraint previously read as a redundant/odd `{>= "3.15" & >= "2.9"}`. It is now a clean single constraint, `"dune" {>= "3.15"}` (`sarek-metal.opam:14`), consistent with the other backend opam files.
...
- ~~Check that `make opam` is idempotent or remove it.~~ **DONE 2026-07-02** — `scripts/check-opam-clean.sh` now asserts this and is wired into the test chain.
...
- ~~Move opam availability constraints into Dune project metadata and remove mutating `Makefile:21-24`.~~ **DONE 2026-07-02** — the mutating lines are removed; `opam:` is now just `dune build @install`.
- Add `make help` output generated from actual targets and test README command snippets. Still open.
- Make Jupyter token/password configurable and default to secure credentials outside Binder. Still open.
- Pin Docker Python/opam dependencies or record a lockfile/switch export for the interactive image. Still open.
- Normalize opam Dune constraints and add an opam lint target in CI. Still open (the `sarek-metal.opam` Dune-constraint fix noted elsewhere in this file was a narrower, already-resolved 2026-06-02 item).
- ~~Define `install_sarek` and `samples` (or remove them from `check`'s prerequisite list at `Makefile:370`) so `make check` does not fail immediately on missing targets.~~ **DONE 2026-07-02** — `check` is now `all test`; `install_sarek`/`samples`/`test_samples_cuda` are gone.
