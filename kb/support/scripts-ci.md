...
- Workflows: `.github/workflows/ci.yml`, `docs.yml`, `deploy-pr-preview.yml`, `cleanup-pr-preview.yml`, `ghcr-image.yml`, `retarget-prs.yml`, `block-master-prs.yml` (added to inventory 2026-07-02), plus workflow docs.
...
- **fixed 2026-07-02 (merged)** — `scripts/check-license-headers.sh` previously ran the mutating fixer (`add-license-headers.sh`) against the real tree and then checked `git diff`, so running the "check" could itself corrupt files and gave false results against a pre-existing dirty tree. Commit `5bad179d` ("make license header check non-mutating") added a `--check`/`--dry-run` mode to `add-license-headers.sh` that computes each candidate file's would-be output in a temp copy and diffs it in place, never touching the real file; `check-license-headers.sh` now delegates to `add-license-headers.sh --check` (confirmed in current source: `if OUTPUT=$("$SCRIPT_DIR/add-license-headers.sh" --check 2>&1); then ...`) instead of shelling out to `git diff`. The same commit fixed the root cause of a related header-mangling bug: the fixer matched a contributor's copyright line by a case-sensitive `grep` on their email, so a header written as `...@Gmail.com` (capital G) never matched the canonical `...@gmail.com` and was treated as a new contributor on every run, appending a duplicate `SPDX-FileCopyrightText` line each time (confirmed: contributor matching and the corresponding year-bump `sed` are now `grep -i`-based, `scripts/add-license-headers.sh:122-153`).
- `scripts/add-license-headers.sh` processes only `scripts ci` for shell scripts (`scripts/add-license-headers.sh:225-245`), so `benchmarks/run_all_benchmarks.sh` and `docker_scripts/*.sh` are outside the fixer/checker scope. Not addressed by this pass — still open.
...
- ~~A test that `check-license-headers.sh` leaves a clean tree when no changes are needed.~~ **Largely moot 2026-07-02** — the script no longer mutates the tree at all (it delegates to a true dry-run mode), so "leaves a clean tree" is now true by construction rather than something to test for regression against. A CI assertion that the dry-run path itself never writes would still be a reasonable addition.
...
- ~~Add a true dry-run mode to `add-license-headers.sh` and make `check-license-headers.sh` use it.~~ **DONE 2026-07-02** — `--check`/`--dry-run` mode added; `check-license-headers.sh` now delegates to it.
