...
- `spoc/registry/dune`: builds public library `spoc.registry` from `Sarek_registry` with an **empty `libraries` list** (`spoc/registry/dune:7`) — it no longer depends on `spoc_framework`.
...
- `cuda_or_opencl (framework : string) cuda_code opencl_code` takes a **framework name string directly**, not a `device` record (`spoc/registry/Sarek_registry.ml:228-232`) — matches `Sarek_pure_registry`'s framework-string convention now that the module has no `spoc_framework` dependency to source a `device` from. It returns `opencl_code` only when `framework = "OpenCL"`; `"CUDA"`, `"Native"`, `"Interpreter"`, and any other string get `cuda_code`.
...
- `cuda_or_opencl` defaults unknown frameworks to CUDA syntax (`spoc/registry/Sarek_registry.ml:228-232`), which may be wrong for Vulkan/Metal/future backends — the `_` catch-all in the match is exactly where a dedicated Vulkan/Metal branch is missing. Still live.
