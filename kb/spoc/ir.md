...
- `spoc/ir/Sarek_ir_codegen.ml` (+ `.mli`): shared GPU code-generation helpers extracted from the backends to avoid duplicated variant/struct emission — `mangle_name`, `gen_variant_def` (C/MSL tagged-union variant emission for CUDA/OpenCL/Metal), `gen_variant_def_glsl` (GLSL variant emission for Vulkan), and the declaration-ORDER surface described below (`type_decl`, `sort_type_decls_by_dependency`, `gen_type_decls`).
...
- `spoc/ir/test/*`: construction, printing, float64 analysis, capability, coopmat, pure-registry and type-declaration-order tests. `spoc/ir/test/dune` lists `test_sarek_ir_types test_sarek_ir_pp test_sarek_ir_analysis test_sarek_capability test_sarek_coopmat test_sarek_pure_registry test_sarek_ir_codegen_decl_order`. (The former claim here — that no test file covered `Sarek_pure_registry.ml`, quoting a three-name `dune` body — went stale when `test_sarek_pure_registry.ml` was added and was corrected on 2026-07-30.)
...
- `Sarek_ir_codegen.ml` also owns type-DECLARATION ORDER, and owns it for every backend that declares struct types (CUDA — and HIP through the CUDA generator — OpenCL, Metal, GLSL, WGSL; PTX declares none). Records and variants are carried in ONE list as `type_decl` (`Record_decl` / `Variant_decl`), ordered by a single topological pass `sort_type_decls_by_dependency`, and emitted by `gen_type_decls ~emit_record ~emit_variant ~tie_break`, which dispatches each entry to the emitter for its own kind; the C family goes through the `gen_c_type_decls` specialisation. `~tie_break` (`Variants_first` for CUDA/OpenCL/Metal/HIP, `Records_first` for GLSL/WGSL) reproduces the order that backend's two former loops produced, so an edge-free kernel's source stays byte-identical. The pass is NOT a stable sort in the general sense — a blocked declaration is overtaken by later independent ones — but a list already in a valid emission order comes back unchanged, which is the property the committed goldens rest on. Node identity is the POSITION in the list, not the type name, so a record's edge to a same-named variant survives. This shape is backlog-211: sorting each kind inside its own loop left the CROSS-KIND edge ordered by neither loop, and each family was red on exactly the shape the other was green on — a variant with a record payload failed on OpenCL (`unknown type name`), a record with a variant-typed field failed on Vulkan (`syntax error, unexpected IDENTIFIER`). Reaching either half from source needs a runtime-selected constructor, because static tag erasure removes a literal-constructor variant before lowering. `referenced_type_names` collects the record AND variant names reachable from a type with a visited set keyed on PHYSICAL node identity, so a cyclic `elttype` value terminates even when neither a record nor a variant sits on the cycle (`let rec t = TVec t`); an unorderable cycle between distinct declarations raises `Type_decl_cycle` rather than emitting a wrong order.
