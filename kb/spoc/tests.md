...
- `spoc/ir/test/dune`: builds three tests against `sarek_ir` (`spoc/ir/test/dune:1-3`). **`Sarek_pure_registry.ml` is part of the `sarek_ir` library (`spoc/ir/dune:4-13`) but has no dedicated test file** — the 4x-duplicated float64 intrinsic table drift documented in [ir.md](ir.md) is untested and not caught by CI.
...
- `Sarek_pure_registry.ml` has zero test coverage: no assertion exercises `fun_device_template`, `register_fun` overwrite semantics, or `glsl_override_name`/`float32_math_template`/`generic_math_template` dispatch. This gap is why the `Math.Float64` table under-registration (see [ir.md](ir.md#sarek_pure_registry-details)) shipped undetected — a per-`(module_path, name, framework)` coverage test would catch it.
