...
- [sarek/tests/](sarek/tests/README.md): unit tests, e2e tests, negative compile tests, new runtime tests, codegen golden snapshots, native placeholder.
...
- `sarek/tests/`: unit, e2e, negative, native, new-runtime, and codegen-golden test suites. (The `common/` shared-helper library was deleted 2026-07-25, task #78 — it had zero consumers.)
...
- Bare `float` kernel-type-annotation drift is resolved (2026-07-02 spec change, human decision: defaults to `float32`, not a bug fix); array unification still ignores memory space (open human decision needed); see [ppx/README.md](sarek/ppx/README.md).
