...
{"id": "PROP-SUITE-COUNT-REFUSES-FAILED-RUN", "type": "grep-present", "description": "The counter must refuse a log whose run did not finish. A `dune test` that dies on a compile error still emits every epilogue of every suite that ran first: measured on this tree, 1692 cases across 116 suites out of a true 1697/117, 0 FAIL, exit 0 while dune exited 1. Emptiness, truncation and the plausibility floor are all satisfied by that log -- the missing fact is not in the counts, so the scan for the runner's own failure report is the only gate that sees it, and it cannot be replaced by any floor loose enough to admit a scoped run. backlog-157.", "check": {"file": "scripts/test-suite-counts.sh", "literal": "runner_failure is not None"}}
{"id": "PROP-SUITE-COUNT-MARKER-IS-DUNES-VOICE", "type": "grep-present", "description": "The runner-failure scan is anchored at column 0 on the premise that column 0 is dune speaking. On this tree it is not, reliably: dune passes an e2e rule stdout through unindented, and a real full-suite log has 65 column-0 lines starting with the word Error while the run is going fine -- `Error executing LLVM compilation action.` from pocl, and `Error on <device>: ...` from sarek/tests/e2e/Benchmarks.ml:247, which prints exactly that for a GPU error it then TOLERATES. So the pattern is `Error` followed by a colon or a space-paren (`Error: ` for a compile error, `Error (warning 32 ...)` for the warnings-as-errors build this dev profile actually fails on), never `^Error\\b`. Widening it turns the counter red on ordinary passing runs. backlog-157 review.", "check": {"file": "scripts/test-suite-counts.sh", "literal": "r\"Error(?::| \\().*\""}}
{"id": "PROP-SUITE-COUNT-COMMAND-MARKER-IS-DUNES-VOICE", "type": "grep-present", "description": "The other half of the marker set, and the half that had no identity guard at all until the backlog-157 close-out. Under `--display verbose` a runtest rule that fails at RUNTIME is reported by dune as `Command [17] exited with code 3:` or `Command [17] got signal SEGV:` -- both measured on this tree with dune 3.24.1, both captured as real fixtures (dune-verbose-command-exit-log.txt, dune-verbose-signal-log.txt). The job number and the exit code are `\\d+` on purpose: dune numbers jobs by scheduling order and reports the process's actual status, so a pattern narrowed to a fixture's own literals would still pass every fixture-driven case. The predecessor of this line, `Command exited with code N.`, matched nothing dune emits in any display mode and was dead from the day it was written -- which is what a pattern asserted in a header and exercised by nothing looks like. backlog-157 close-out.", "check": {"file": "scripts/test-suite-counts.sh", "literal": "r\"|Command \\[\\d+\\] (?:exited with code \\d+|got signal \\S+):.*\""}}
